...
## I. Lấy link bản thiết kế - Tương tự Dentalshare Exocad

   1. Tải và mở file `Huong_Cam_Tao_Viewer.exe` trên máy tính ( phần mềm sẽ chạy dưới dạng Portable và chạy nền). Lưu file ở 1 vị trí cố định trên máy tính, ưu tiên tạo 1 thư mục riêng để chứa các thông tin config.
   2. Cấu hình
   ...
   - Khi muốn lấy link để gửi. Trong Exocad Vào mục **Tool - Export html** . Lưu file **html** vào thư mục bên trong dự án Exocad đang làm (mặc định Exocad sẽ tự nhận diện).
     Lưu ý: Tên file sẽ là tiêu đề của dự án( mặc định đặt theo tên thư mục đã cấu hình). 
   - Phần mềm sẽ tự động xử lý, trung bình dưới 10s sẽ có thông báo gửi về và cung cấp đường dẫn link và tự động lưu link vào bộ nhớ tạm.
   - Với các bản thiết kế sau chỉnh sửa, hãy ghi đè lại file `html`. Hệ thống sẽ tự động cập nhật lại bản thiết kế theo link ban đầu.
     Lưu ý :
           -  Nếu đổi tên file thiết kế sau chỉnh sửa/ có nhiều file `html` trong cùng 1 folder: Hệ thống sẽ gộp chung các file html vào chung 1 link ( nhiều file html sẽ hiển thị cùng 1 lúc)
           -  Hệ thống sẽ không tự động xóa các file trên server nếu file đã xóa trên máy tính (chỉ hỗ trợ cập nhật nếu file bị chỉnh sửa). Vì vậy nếu muốn xóa project cần đăng nhập vào website để thực hiện.


       <img width="361" height="179" alt="image" src="https://github.com/user-attachments/assets/a8179266-b20d-44fc-979b-c7bc3e12d4c8" />
   
   - Lấy link chia sẻ : nhấn chuột phải vào biểu tượng dưới **Taskbar** -> **Lịch sử Upload** để xem danh sách các project đã upload lên và danh sách còn tồn tại trên hệ thống để copy lại các link khi cần thiết
...
Với các bác muốn tùy chỉnh chuyên sâu thì cần đăng nhập website để thực hiện các tính năng nâng cao khác
   ...
   - Thêm thủ công các file html/stl/obj/ply vào dự án khi cần thiết ( do hiện tại công cụ chỉ hỗ trợ Upload tự động html)
...
- **Bảng điều khiển (Layers)**: Nhấp vào biểu tượng  ở góc trái màn hình để hiện/ẩn danh sách các thành phần.
- **Thanh điều hướng** : Hỗ trợ trong quá trình xoay hình. Tương thích với hệ trục tọa độ máy Shinning3D khi căn chỉnh đúng. Có thể dùng Cube ở góc phải bên dưới màn hình để thực hiện với tính năng tương đương.
...
- **Thêm file**: Bổ sung thêm file. Hỗ trợ định dạng : `html`, `stl`,`obj` ,`ply`.
---
